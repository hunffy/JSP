...
<%--/jsp2/src/main/webapp/model1/member/info.jsp 
    1. id 파라미터값을 변수에 저장하기. 
    2. login 상태 검증.
    	로그아웃상태 : 로그인하세요 메세지출력 후 loginForm.jsp 페이지 이동
    3. login 상태 검증 2.
    	로그인상태 : 관리자가 아닌 경우 id 파라미터값과 login정보가 다르면
...
<% 
	...
	String id = request.getParameter("id"); //	1번부분. id 파라미터 저장
...
<%--관리자이거나, 본인정보인경우 --%>
<% }else { //정상적인 경우 :관리자인 경우나, 본인정보를 조회할때
...
<tr><td rowspan="6" whidth="30%">
<img src="picture/<%=mem.getPicture() %>" width="200" height="210">
</td>
	<th witdh="20%">아이디</th><td><%=mem.getId() %></td></tr>
