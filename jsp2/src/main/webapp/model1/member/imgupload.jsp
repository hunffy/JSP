...
<%--/jsp2/src/main/webapp/model1/member/imgupload.jsp 
	...
	2. 이미지파일을 opner 페이지에 출력하기. 현재페이지는 닫음
--%>

<% //1. 파일업로드
	//uploadPath : 파일 업로드되는 폴더이름
	String uploadPath = application.getRealPath("/")+
						"model1/member/picture/";
//	File : 파일의 정보를 관리하는 클래스
	File f = new File(uploadPath);
//f.exists() : f가 존재유무 물어봄
//mkdirs() : 폴더 생성해.
	if(!f.exists()) 
		f.mkdirs(); //이미지파일 업로드 되는 폴더 생성해주는 부분
	...
	try{
		/*
			...
			request : 요청정보. 클라이언트에서 선택된 파라미터정보,파일이름,파일의 내용등이 저장된 요청객체.
			
			uploadPath : 파일이 업로드되는 폴더. 업로드 위치폴더
			10*1024*1024 : 10M(메가). 업로드 되는 파일의 최대 크기 지정.
			"utf-8" : 파라미터 인코딩 코드
		...
		*/
		...
		// memberimg.jsp의 <input type="file" name="picture" 
		filename = multi.getFilesystemName("picture");//파일이름을 가져온것.
	}catch(IOException e){
		e.printStackTrace();
	}

%>
<%-- 2.이미지파일을 opener 페에지에 출력하기. 현재페이지 닫음. --%>
<script>
