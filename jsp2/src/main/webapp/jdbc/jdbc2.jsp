...
<%@ page language="java" contentType="text/html; charset=UTF-8"
    ...
    <%--
    	JDBC 사용순서
    	1. 드라이버 클래스 로드 	: Class.formName("클래스명")
    	2. db와 연결하는 객체  	: Connection conn = DriverManager.getConnection("url","userid","pw")
    	3. db에 sql 구문을 전달하는 객체 Statement stmt = conn.createStatement()
...
<%/*
	...
	getColumnName(i) : 조회된 컬럼명의 순서대로 조회컬럼 리턴. 1번 인덱스부터 시작.
*/
	for(int i=1; i<=rsmt.getColumnCount(); i++){
%>
<th><%=rsmt.getColumnName(i) %></th> <%--제일 윗줄 컬럼명 출력되는 부분 --%>
<% }%> </tr>
